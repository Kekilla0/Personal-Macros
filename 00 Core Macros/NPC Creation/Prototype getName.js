function getName({race = ``, sex = ``} = {}){
  ...
  if (!sex || sex.toLowerCase() === "random") sex = getSex();
  if (!race || race.toLowerCase() === "random") race = getRace();

  let output = duplicate(data.find(ele=> race.toLowerCase().includes(ele.name.toLowerCase())));

  if(output.parent !== null){
    let prnt = output.parent.shuffle().random();
    output.lastName = duplicate(data.find(ele=> ele.name === prnt).lastName);
  }

  let lnLength = Math.floor(Math.random()* output.lastName.length);
  lnLength = lnLength > 5 ? lnLength : 5;

  return {
    name : {
      first : output.firstName[sex].map(arr=> arr.shuffle().random()).join(``).capitalize(),
      last : output.lastName.map(arr=> arr.shuffle().random()).filter((e,i)=> i < lnLength ).join(``).capitalize(),
    },
    race, 
    sex
  };
